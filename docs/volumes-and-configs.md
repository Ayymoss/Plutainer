...
The SteamCMD games (`7dtd`, `cs2`, `l4d2`, `hl2dm`) do not use `/home/plutainer/gamefiles` at all; SteamCMD installs them under the app volume instead. Nebula does use it: mount the root of your owned Dyson Sphere Program installation. Its bundled headless Steam API layer is copied only into `app/runtime/`; it never replaces the original DLL in this read-only mount.
...
app/
  ...
  runtime/
    ...
    nebula/nebula/    ← writable DSP + BepInEx/Nebula overlay
  .plutainer-version  ← layout marker
```

You edit `configs/`. You read `logs/`. You can ignore `runtime/`.

For Nebula, `app/configs/` is linked as BepInEx's whole config directory, so `nebula.cfg`, `nebulaGameDescSettings.cfg`, `BepInEx.cfg` and configs generated by extra mods all persist there. DSP saves live at `app/runtime/gamedata/nebula/Dyson Sphere Program/Save/`.
...
Verified on the eleven pre-Nebula server types, and confirmed on a live CoD4x server: after truncation a map change wrote a fresh `InitGame:` line to the emptied file. Nebula disables copy-truncate rotation because Unity's file writer is not known to reopen in append mode.
